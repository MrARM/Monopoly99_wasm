...
typedef struct {
	iRECT	clip;		/* clip area */
	short	ofs[2];		/* drawing offset */
	iRECT	tw;		/* texture window */
...
typedef struct {
	iRECT	disp;		/* display area */
	iRECT	screen;		/* display start point */
...
typedef struct {
	...
	iRECT	*crect;		/* CLUT rectangle on frame buffer */
	u_long	*caddr;		/* CLUT address on main memory */
	iRECT	*prect;		/* texture image rectangle on frame buffer */
...
extern int ClearImage(iRECT *rect, u_char r, u_char g, u_char b);
extern int ClearImage2(iRECT *rect, u_char r, u_char g, u_char b);
...
extern void SetDrawArea(DR_AREA *p, iRECT *r);
extern void SetDrawEnv(DR_ENV *dr_env, DRAWENV *env);
extern void SetDrawLoad(DR_LOAD *p, iRECT *rect);
extern void SetDrawMode(DR_MODE *p, int dfe, int dtd, int tpage, iRECT *tw);
extern void SetDrawTPage(DR_TPAGE *p, int dfe, int dtd, int tpage);
extern void SetDrawMove(DR_MOVE *p, iRECT *rect, int x, int y) ;
...
extern void SetTexWindow(DR_TWIN *p, iRECT *tw);
...
extern int LoadImage2(iRECT *rect, u_long *p);
extern int StoreImage2(iRECT *rect, u_long *p);
extern int MoveImage2(iRECT *rect, int x, int y);
