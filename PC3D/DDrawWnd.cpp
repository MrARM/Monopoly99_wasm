...
#define WEB_DDRAW_LOG(message) monopoly_web_log(message)
#else
#define WEB_DDRAW_LOG(message) ((void)0)
#endif
...
DDrawDriver* DDrawWnd::FindDDDriver(DDDrvMgr& ddDrvMgr, DWORD dwRequiredMem)
{
  ...
  if (!DXINI_FileExists())
  {
    ...
    if (!m_bINIFileGenerated)
    {
      ...
      {
        WEB_DDRAW_LOG("DDrawWnd: DirectX.ini generation failed.");
        return(NULL);
      }
    }
  }
  ...
  if (!pDDDriver)
  {
    ...
    if (m_bINIFileGenerated)
    {
      WEB_DDRAW_LOG("DDrawWnd: generated DirectX.ini but could not read DDraw driver.");
      return(NULL);
    }
    ...
    else
    {
      ...
      {
        WEB_DDRAW_LOG("DDrawWnd: DirectX.ini regeneration failed.");
        return(NULL);
      }
    }
  }
...
D3DDevice* DDrawWnd::FindD3DDevice(DDDrvMgr& ddDrvMgr, DWORD dwRequiredMem)
{
  ...
  if (!pD3DDevice)
  {
    ...
    if (m_bINIFileGenerated)
    {
      WEB_DDRAW_LOG("DDrawWnd: generated DirectX.ini but could not read D3D device.");
      return(NULL);
    }
    else
    {
                      ...
                      WindowedMode nWindowMode /*= WINDOWED_USE_DEFAULT*/)
{
  ...
  ASSERT(NULL != hWnd); if (!hWnd)
  {
    WEB_DDRAW_LOG("DDrawWnd: missing main window handle.");
    return(FALSE);
  }
  m_hWindow = hWnd;
  ...
  ASSERT(NULL != m_pDDDriver); if (!m_pDDDriver)
  {
    WEB_DDRAW_LOG("DDrawWnd: no DirectDraw driver selected.");
  ...
  ASSERT(NULL != m_pD3DDevice); if (!m_pD3DDevice)
  {
    WEB_DDRAW_LOG("DDrawWnd: no D3D device selected.");
    goto CREATION_FAILED;
  }
  ...
  ASSERT(m_bInitialized); if (!m_bInitialized)
  {
    WEB_DDRAW_LOG("DDrawWnd: SetDisplayMode failed.");
    goto CREATION_FAILED;
  }
                              ...
                              UINT nClrDepth /*= 0*/, BOOL bUseZBuffer /*= TRUE*/)
{
  ...
  if (!m_pDDDriver->SupportsDisplayMode(nWidth, nHeight, nClrDepth))
  {
    ...
    MSGBOX(szError, "FATAL ERROR!");
    WEB_DDRAW_LOG("DDrawWnd: requested display mode is unsupported.");
    return(FALSE);
  }
  ...
  if (IsWindowed())
  {
    ...
    {
      WEB_DDRAW_LOG("DDrawWnd: windowed cooperative level failed.");
  ...
  else
  {
    ...
    if (!m_pDDDriver->SetCooperativeLevel(dwValue | DDSCL_EXCLUSIVE | 
                                          DDSCL_FULLSCREEN))
    {
      WEB_DDRAW_LOG("DDrawWnd: fullscreen cooperative level failed.");
      goto SET_DISPLAY_FAILED;
    }
    ...
    if (!m_pDDDriver->SetDisplayMode(nWidth, nHeight, nClrDepth))
    {
      ...
      sprintf(szError, "Failed to set the display mode to %d x %d x %d.",
              nWidth, nHeight, nClrDepth);
      MSGBOX(szError, "FATAL ERROR!");
      WEB_DDRAW_LOG("DDrawWnd: fullscreen SetDisplayMode call failed.");
      goto SET_DISPLAY_FAILED;
    }
  }
  ...
  if (!CreateSurfaces(nWidth, nHeight))
  {
    ...
    MSGBOX(szError, "FATAL ERROR!");
    WEB_DDRAW_LOG("DDrawWnd: CreateSurfaces failed.");
    goto SET_DISPLAY_FAILED;
  }
  ...
  {
    WEB_DDRAW_LOG("DDrawWnd: render target validation failed.");
    goto SET_DISPLAY_FAILED;
  }
  ...
  if (bUseZBuffer)
  {
    if (!CreateZBuffer())
    {
      ...
      MSGBOX(szError, "FATAL ERROR!");
      WEB_DDRAW_LOG("DDrawWnd: CreateZBuffer failed.");
      goto SET_DISPLAY_FAILED;
    }
  }

  // Now create the Direct3D device
  if (!m_pD3DDevice->InitD3D(&m_Render))
  {
    WEB_DDRAW_LOG("DDrawWnd: D3D device initialization failed.");
    goto SET_DISPLAY_FAILED;
  }
