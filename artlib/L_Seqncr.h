...
extern "C" void *monopoly_web_audio_play_wav_data(const unsigned char *data, unsigned int size,
  int loop, int volume, int pan);
extern "C" void monopoly_web_audio_stop_handle(void *handleValue);
extern "C" void monopoly_web_audio_set_volume_handle(void *handleValue, int volume);
extern "C" void monopoly_web_audio_set_pan_handle(void *handleValue, int pan);
extern "C" void monopoly_web_audio_set_loop_handle(void *handleValue, int loop);
extern "C" unsigned int monopoly_web_audio_get_position_ticks_handle(void *handleValue,
  unsigned int ticksPerSecond);
extern "C" unsigned int monopoly_web_audio_wav_duration_ticks(const unsigned char *data,
  unsigned int size, unsigned int ticksPerSecond);

typedef struct LE_SOUND_AudioStreamStruct {
  LE_SOUND_BufSndHandle bufSnd;
  UNS32 duration;
  BOOL doNotFeed;
  UNS32 playTime;
  ...
  INT32 startPosition;
  INT32 endPosition;
  INT32 jumpTriggerPosition;
  INT32 jumpToPosition;
  UNS32 jumpCount;
  BOOL jumpCutRequested;
} LE_SOUND_AudioStreamRecord, *LE_SOUND_AudioStreamPointer;
static const int LE_SOUND_CurrentHardwareHz = 22050;
static const int LE_SOUND_CurrentHardwareBits = 16;
static inline BOOL LE_SOUND_OpenAudioStream(LE_SOUND_AudioStreamPointer AudioPntr,
  char *, LE_DATA_DataId WaveDataID, BOOL, BOOL StartPaused, BOOL)
{
  if (AudioPntr != NULL) {
    void *waveData = NULL;
    UNS32 waveSize = 0;
    UNS32 duration = 1;
    LE_SOUND_BufSndHandle handle = NULL;
    if (WaveDataID != LE_DATA_EmptyItem) {
      waveData = LE_DATA_Use(WaveDataID);
      waveSize = LE_DATA_GetLoadedSize(WaveDataID);
      if (waveData != NULL && waveSize > 0) {
        duration = monopoly_web_audio_wav_duration_ticks((const unsigned char *)waveData,
          (unsigned int)waveSize, CE_ARTLIB_TIME_BASIC_CLOCK_RATE_HZ);
        if (!StartPaused) {
          handle = monopoly_web_audio_play_wav_data((const unsigned char *)waveData,
            (unsigned int)waveSize, FALSE, 100, 0);
        }
      }
    }
    AudioPntr->bufSnd = NULL;
    AudioPntr->bufSnd = handle;
    AudioPntr->duration = duration;
    AudioPntr->doNotFeed = TRUE;
    AudioPntr->playTime = 0;
    AudioPntr->currentPosition = 0;
    AudioPntr->startPosition = 0;
    AudioPntr->endPosition = (INT32)duration;
    AudioPntr->jumpTriggerPosition = 0;
    AudioPntr->jumpToPosition = 0;
    AudioPntr->jumpCount = 0;
    AudioPntr->jumpCutRequested = FALSE;
  }
  return TRUE;
}
static inline BOOL LE_SOUND_FeedAudioStream(LE_SOUND_AudioStreamPointer,
  BOOL) { return TRUE; }
static inline void LE_SOUND_CloseAudioStream(LE_SOUND_AudioStreamPointer AudioPntr) {
  if (AudioPntr != NULL && AudioPntr->bufSnd != NULL) {
    monopoly_web_audio_stop_handle(AudioPntr->bufSnd);
    AudioPntr->bufSnd = NULL;
  }
}
static inline BOOL LE_SOUND_SetVolumeBufSnd(LE_SOUND_BufSndHandle BufSndHandle, UNS8 Volume) {
  if (BufSndHandle != NULL) monopoly_web_audio_set_volume_handle(BufSndHandle, (int)Volume);
  return TRUE;
}
static inline BOOL LE_SOUND_SetPanBufSnd(LE_SOUND_BufSndHandle BufSndHandle, INT8 Pan) {
  if (BufSndHandle != NULL) monopoly_web_audio_set_pan_handle(BufSndHandle, (int)Pan);
  return TRUE;
}
static inline BOOL LE_SOUND_SetPitchBufSnd(LE_SOUND_BufSndHandle, UNS32) { return TRUE; }
static inline BOOL LE_SOUND_SetLoopBufSnd(LE_SOUND_BufSndHandle BufSndHandle, BOOL Loop) {
  if (BufSndHandle != NULL) monopoly_web_audio_set_loop_handle(BufSndHandle, Loop ? 1 : 0);
  return TRUE;
}
static inline BOOL LE_SOUND_PauseBufSnd(LE_SOUND_BufSndHandle, BOOL) { return TRUE; }
static inline BOOL LE_SOUND_StopBufSnd(LE_SOUND_BufSndHandle BufSndHandle) {
  if (BufSndHandle != NULL) monopoly_web_audio_stop_handle(BufSndHandle);
  return TRUE;
}
static inline INT32 LE_SOUND_GetTimeOfBufSnd(LE_SOUND_BufSndHandle BufSndHandle) {
  if (BufSndHandle == NULL) return 0;
  return (INT32)monopoly_web_audio_get_position_ticks_handle(BufSndHandle,
    CE_ARTLIB_TIME_BASIC_CLOCK_RATE_HZ);
}
static inline UNS32 LE_SOUND_GetSoundDuration(LE_DATA_DataId WaveDataID) {
  if (WaveDataID == LE_DATA_EmptyItem) return 1;
  void *waveData = LE_DATA_Use(WaveDataID);
  UNS32 waveSize = LE_DATA_GetLoadedSize(WaveDataID);
  if (waveData == NULL || waveSize == 0) return 1;
  return monopoly_web_audio_wav_duration_ticks((const unsigned char *)waveData,
    (unsigned int)waveSize, CE_ARTLIB_TIME_BASIC_CLOCK_RATE_HZ);
}
static inline LE_SOUND_BufSndHandle LE_SOUND_StartBufSnd(void *, void *,
  char *, BOOL, LE_DATA_DataId WaveDataID, BOOL, BOOL *, BOOL Loop, int, int) {
  if (WaveDataID == LE_DATA_EmptyItem) return NULL;
  void *waveData = LE_DATA_Use(WaveDataID);
  UNS32 waveSize = LE_DATA_GetLoadedSize(WaveDataID);
  if (waveData == NULL || waveSize == 0) return NULL;
  return monopoly_web_audio_play_wav_data((const unsigned char *)waveData,
    (unsigned int)waveSize, Loop, 100, 0);
}
static inline BOOL LE_SOUND_FeedBufSnd(LE_SOUND_BufSndHandle, DWORD, BYTE *,
  BOOL *, DWORD, BYTE *, DWORD *, BOOL *, INT32, INT32, BOOL) { return TRUE; }
#endif
...
#if MONOPOLY_WEB
inline BOOL LE_SEQNCR_TakeOverAndPlayBinkVideo (char *, BOOL = FALSE) { return TRUE; }
#elif CE_ARTLIB_EnableSystemBinkStandAlone
