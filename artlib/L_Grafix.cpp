...
static void LI_GRAFIX_SetGlobalScreenFormatFromPrimarySurface (void)
{
  ...
  // Shift values are the index of the least-significant set bit in each mask.
  Count = 0;
  while (Count < 32 && ((LE_BLT_dwRedMask >> Count) & 1) == 0) Count++;
  LE_BLT_nRedShiftValue = Count;

  Count = 0;
  while (Count < 32 && ((LE_BLT_dwGreenMask >> Count) & 1) == 0) Count++;
  LE_BLT_nGreenShiftValue = Count;

  Count = 0;
  while (Count < 32 && ((LE_BLT_dwBlueMask >> Count) & 1) == 0) Count++;
