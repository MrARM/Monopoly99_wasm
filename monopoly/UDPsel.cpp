 ...
 int maxGreatestNetWorthLength, int numberOfHiScores )
{
  ...
  for ( rank = 0; rank < numberOfHiScores; rank++ )
  {
    ...
    for ( i = 0; i < udpsel_HISTORYLOG_MAX_PLAYERS; i++ )
    {
      ...
      if ( wins < 1 )
        continue;
      GetPrivateProfileString( playerKeyString, udpsel_HISTORYLOG_NETWORTHKEY, "", tempString, sizeof(tempString),
       LE_DIRINI_INI_PathName );
...
void UDPSEL_GameHasJustStarted( void )
{
  ...
  for ( i = 0; i < UICurrentGameState.NumberOfPlayers; i++ )
  {
    ...
    for ( j = 0; j < udpsel_HISTORYLOG_MAX_PLAYERS; j++ )
    {
      ...
      GetPrivateProfileString( playerKeyString, udpsel_HISTORYLOG_NAMEKEY, "", tempString, sizeof(tempString),
       LE_DIRINI_INI_PathName );
      if ( strcmp(tempString,"") == 0 )
      {
...
void UDPSEL_ProcessMessage( LE_QUEUE_MessagePointer pUIMessage )
{
   ...
   || (DISPLAY_state.desired2DView == DISPLAY_SCREEN_PselectRules) )
  {
    ...
    if ( UIMSG_MOUSE_LEFT_DOWN == pUIMessage->messageCode )
    {
      ...
      {
        char buffer[224];
        sprintf(buffer, "UDPSEL click x=%ld y=%ld button=%d phase=%d desiredPhase=%d standard=%d custom=%d view=%d desired=%d",
          (long) mousePoint.x,
          (long) mousePoint.y,
          buttonIndex,
          DISPLAY_state.currentPlayerSetupPhase,
          DISPLAY_state.desiredPlayerSetupPhase,
          (int) udpsel_OBJ_RULESCHOICE_BUTTON_STANDARD,
          (int) udpsel_OBJ_RULESCHOICE_BUTTON_CUSTOM,
