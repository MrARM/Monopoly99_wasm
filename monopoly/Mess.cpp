...
static BOOL MESS_WebShouldLogAction(RULE_ActionType action)
{
  switch (action)
  {
...
static void MESS_WebLogAction(const char *prefix, RULE_ActionArgumentsPointer message)
{
  char buffer[192];
  sprintf(buffer, "%s action=%d from=%d to=%d A=%ld B=%ld C=%ld D=%ld",
    prefix,
    (int) message->action,
    (int) message->fromPlayer,
    (int) message->toPlayer,
    message->numberA,
    message->numberB,
    message->numberC,
    message->numberD);
...
{
#if MONOPOLY_WEB
   if (MESS_WebShouldLogAction(MessageToSend->action))
      MESS_WebLogAction("MESS send", MessageToSend);
#endif
    ...
    if (FromID == DPID_SYSMSG) /* A system message. */
    {
      ...
      case DPSYS_CHAT:
        ...
        if (ChatMemoryHandle == NULL)
          break; /* Out of memory. */
        ChatMemoryPntr = (wchar_t *) RULE_LockHandle (ChatMemoryHandle);
...
#if MONOPOLY_WEB
  if (bRet && MESS_WebShouldLogAction(MessageToReceive->action))
    MESS_WebLogAction("MESS recv", MessageToReceive);
#endif
