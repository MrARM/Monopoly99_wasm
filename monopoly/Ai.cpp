...
void AI_Send_Config_Accepts(RULE_ActionArgumentsPointer NewMessage)
{
  ...
  {
    char buffer[128];
    sprintf(buffer, "AI config accepts pendingSet=0x%lx", (unsigned long) players);
  ...
  for(cur_player = 0; cur_player < RULE_MAX_PLAYERS; ++cur_player)
  {
    ...
    {
      char buffer[160];
      sprintf(buffer, "AI config player=%d pending=%d num_actions=%d",
        (int) cur_player,
        ((1 << cur_player) & players) ? 1 : 0,
