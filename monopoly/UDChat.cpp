...
int CHAT_WordWrap(wchar_t *line, int width, wchar_t **wrapped, int font)
{
...
#if MONOPOLY_WEB
  const int maxLines = (int)(sizeof(CHAT_WrappedLines) / sizeof(CHAT_WrappedLines[0]));
  const int lineCapacity = (int)(sizeof(CHAT_WrappedLines[0]) / sizeof(CHAT_WrappedLines[0][0]));
  ...
  if (line == NULL)
  {
    CHAT_WrappedLines[0][0] = 0;
    LE_FONTS_GetSettings(0);
    return 0;
  }

  remaining = line;
  while (*remaining != 0 && i < maxLines)
  {
    const wchar_t *start = remaining;
    size_t segmentLength = 0;
    size_t take = 0;
    size_t copyLength;

    while (start[segmentLength] != 0 && start[segmentLength] != L'\n' &&
      start[segmentLength] != L'\r' && segmentLength < (size_t)(lineCapacity - 1))
      segmentLength++;

    if (segmentLength == 0 && (*start == L'\n' || *start == L'\r'))
    {
      remaining++;
      continue;
    }
    ...
    for (j = 0; j < (int)take; j++)
      CHAT_WrappedLines[i][j] = start[j];
    CHAT_WrappedLines[i][take] = 0;

    if (LE_FONTS_GetStringWidth(CHAT_WrappedLines[i]) > width)
    {
      size_t best = take;
      size_t spaceBreak = 0;
      ...
      while (best > 0)
      {
        for (j = 0; j < (int)best; j++)
          CHAT_WrappedLines[i][j] = start[j];
        CHAT_WrappedLines[i][best] = 0;
        if (LE_FONTS_GetStringWidth(CHAT_WrappedLines[i]) <= width)
          break;
        best--;
      }
      ...
      for (copyLength = best; copyLength > 0; copyLength--)
      {
        if (start[copyLength - 1] == L' ')
        {
          spaceBreak = copyLength;
          break;
        }
      }
    ...
    copyLength = take;
    while (copyLength > 0 && start[copyLength - 1] == L' ')
      copyLength--;
    ...
    for (j = 0; j < (int)copyLength; j++)
    {
      CHAT_WrappedLines[i][j] = (start[j] == A_T('_')) ? A_T(' ') : start[j];
    }
    CHAT_WrappedLines[i][copyLength] = 0;
...
/*bleeble: Add this back when we get approved art*/
  ...
  int i;
  for (i = 0; i < ((CHAT_winsize.y-19) / 5)-1; i++) {
