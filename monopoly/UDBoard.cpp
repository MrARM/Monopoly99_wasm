...
static void UDBOARD_WebLogState(const char *message)
{
  char buffer[192];
  sprintf(buffer, "%s desired=%d current=%d game3D=%d board3D=%d visible=%d viewport=%d camera=%d",
    message,
    DISPLAY_state.desired2DView,
    DISPLAY_state.current2DView,
    DISPLAY_state.game3Don,
    DISPLAY_state.board3Don,
    DISPLAY_IsBoardVisible ? 1 : 0,
    DISPLAY_state.viewportInUse,
    DISPLAY_state.desired2DCamera);
...
#if MONOPOLY_WEB
  if( EM_ASM_INT({
    return new URLSearchParams(window.location.search).has("force2d") ? 1 : 0;
  }) )
  {
    DISPLAY_state.game3Don = FALSE;
    monopoly_web_log("UDBOARD initialize: force2d diagnostic enabled");
  }
  UDBOARD_WebLogState("UDBOARD initialize");
#endif
...
#if MONOPOLY_WEB
  static BOOL loggedMainEntry = FALSE;
  if( !loggedMainEntry && DISPLAY_state.desired2DView == DISPLAY_SCREEN_MainA )
  {
    loggedMainEntry = TRUE;
    UDBOARD_WebLogState("UDBOARD show MainA requested");
  }
#endif
  ...
  { // shut off current view
    ...
    { // turn on new view
      ...
      if( DISPLAY_state.desired2DView == DISPLAY_SCREEN_MainA )
        UDBOARD_WebLogState("UDBOARD activating MainA camera");
#endif
...
#if MONOPOLY_WEB
      UDBOARD_WebLogState("UDBOARD starting 3D board");
#endif
