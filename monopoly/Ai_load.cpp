...
static void AI_WebLowerBasename(const char *input, char *output, size_t outputSize)
{
    ...
    size_t i = 0;
    for (; name[i] && i + 1 < outputSize; ++i)
        output[i] = (char)tolower((unsigned char)name[i]);
    output[i] = 0;
}

static FILE *AI_WebOpenStrategyFile(const char *originalName)
{
    char fileName[_MAX_PATH];
    char path[_MAX_PATH * 2];

    AI_WebLowerBasename(originalName, fileName, sizeof(fileName));
    if (fileName[0] == 0)
        return NULL;
    ...
    for (size_t i = 0; i < sizeof(prefixes) / sizeof(prefixes[0]); ++i)
    {
        ...
        {
            char message[256];
            snprintf(message, sizeof(message), "Loaded AI strategy %s", path);
    ...
    char message[256];
    snprintf(message, sizeof(message), "AI strategy not found: %s", fileName);
    monopoly_web_log(message);
    return NULL;
}
#endif
...
//    #endif
	...
	if((stream = fopen(expanded_filename, "r")) == NULL)
    	{
#if MONOPOLY_WEB
        stream = AI_WebOpenStrategyFile(mbs_filename);
        if(stream == NULL)
#endif
        {
        #if FORHOTSEAT
