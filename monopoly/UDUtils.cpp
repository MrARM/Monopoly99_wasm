...
static void UDUTILS_ApplyWebDisplayDefaults()
{
  ZeroMemory(&DISPLAY_RenderSettings, sizeof(DISPLAY_RenderSettingsStruct));

  DISPLAY_RenderSettings.bClassic_UseHiResMesh = FALSE;
  DISPLAY_RenderSettings.bClassic_UseHiResTextures = FALSE;
  DISPLAY_RenderSettings.bClassic_UseDithering = TRUE;
  DISPLAY_RenderSettings.bClassic_UseBilinearFiltering = TRUE;

  DISPLAY_RenderSettings.bCity_UseHiResMesh = FALSE;
  DISPLAY_RenderSettings.bCity_UseHiResTextures = FALSE;
  DISPLAY_RenderSettings.bCity_UseDithering = TRUE;
  DISPLAY_RenderSettings.bCity_UseBilinearFiltering = TRUE;

  DISPLAY_RenderSettings.bUse3DBoard = TRUE;
  DISPLAY_RenderSettings.bUseBackground = FALSE;
...
BOOL UDUTILS_LoadBoardTextureSet(Mesh* pMesh, const BoardTextureSet& txtrSet)
{
  ...
  // Load the photos
  int i;
  for (i = 0; i < txtrSet.nNumCityPhotos; i++)
  {
...
#if MONOPOLY_WEB
  if (0 == GetPrivateProfileString(CLASSIC_BOARD, USE_HIRES_MESH, TEXT(""),
                                   szValue, 20, szINIFile))
  {
    UDUTILS_ApplyWebDisplayDefaults();
    monopoly_web_log("Monopoly.ini missing; using WebGL 3D display defaults.");
    return(TRUE);
  }
#endif
...
#if MONOPOLY_WEB
  UDUTILS_ApplyWebDisplayDefaults();
  monopoly_web_log("Skipping legacy DirectDraw render speed test; using WebGL 3D display defaults.");
  return(TRUE);
#endif
