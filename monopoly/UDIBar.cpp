...
static void WEB_IBAR_Log(const char *message) { monopoly_web_log(message); }
#else
static void WEB_IBAR_Log(const char *) { }
#endif
...
// Note the font is set by display_initialize, we just set the weight & size.
...
#if MONOPOLY_WEB
  {
    ...
    static int lastCurrent = -999;
    static int lastRule = -999;
    static int lastPlayer = -999;
    static int lastView = -999;
    if (lastVisible != (int)DISPLAY_IsIBarVisible ||
        lastGame != (int)GameInProgress ||
        lastCurrent != DISPLAY_state.IBarCurrentState ||
        lastRule != DISPLAY_state.IBarLastRuleState ||
        lastPlayer != DISPLAY_state.IBarCurrentPlayer ||
        lastView != DISPLAY_state.desired2DView)
    {
      char msg[192];
      sprintf(msg, "UDIBAR show visible=%d game=%d current=%d rule=%d player=%d view=%d",
        (int)DISPLAY_IsIBarVisible, (int)GameInProgress, DISPLAY_state.IBarCurrentState,
        DISPLAY_state.IBarLastRuleState, DISPLAY_state.IBarCurrentPlayer, DISPLAY_state.desired2DView);
      WEB_IBAR_Log(msg);
      lastVisible = (int)DISPLAY_IsIBarVisible;
      lastGame = (int)GameInProgress;
      lastCurrent = DISPLAY_state.IBarCurrentState;
      lastRule = DISPLAY_state.IBarLastRuleState;
      lastPlayer = DISPLAY_state.IBarCurrentPlayer;
...
/*  if ( (DISPLAY_IsIBarVisible) ||// Not really related to IBAR, temp dialogs.
  ...
  {// Update required
    ...
    if( tempID != LED_EI )
    {
      ...
      {
        char msg[128];
        sprintf(msg, "UDIBAR backdrop start id=0x%08X y=%d", (unsigned int)DISPLAY_state.CurrentIbarBackdrop, UDIBAR_IBarTop);
        WEB_IBAR_Log(msg);
      }
#endif
    }
  ...
  for( t = 0; t < IBAR_BUTTON_DISTINCT_MAX; t++ )
  {
    ...
    {// Corrective action.  Mouseover logic dropped in new system.
      ...
      if( DISPLAY_state.IBarButtonsShown[t] != LED_EI )
      {
        ...
        {
          char msg[160];
          sprintf(msg, "UDIBAR button start index=%d visual=%d id=0x%08X pri=%u",
            t, DISPLAY_state.IBarButtonsVisualState[t], (unsigned int)DISPLAY_state.IBarButtonsShown[t], (unsigned int)tempPriority);
          WEB_IBAR_Log(msg);
        }
#endif
        switch( DISPLAY_state.IBarButtonsVisualState[t] )
