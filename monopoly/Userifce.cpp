...
#define WEB_INIT_LOG(message) monopoly_web_log(message)
#else
#define WEB_INIT_LOG(message) ((void)0)
#endif
...
{
  WEB_INIT_LOG("MainExtendedInitialization: mounting core data files.");
  LE_DATA_InitDatafile( "Dat_Mon\\dat_main.dat", DAT_MAIN );
  WEB_INIT_LOG("MainExtendedInitialization: dat_main mounted.");
  LE_DATA_InitDatafile( "Dat_Mon\\dat_pat.dat" , DAT_PAT );
  WEB_INIT_LOG("MainExtendedInitialization: dat_pat mounted.");
//  LE_DATA_InitDatafile( "Dat_Mon\\dat_cge.dat" , DAT_CGE );
#if USA_VERSION
  LE_DATA_InitDatafile( "Dat_Mon\\dat_bord.dat", DAT_BOARD );
  WEB_INIT_LOG("MainExtendedInitialization: dat_bord mounted.");
#else
  LE_DATA_InitDatafile( "Dat_Mon\\dat_borde.dat", DAT_BOARD );
  WEB_INIT_LOG("MainExtendedInitialization: dat_borde mounted.");
#endif
  LE_DATA_InitDatafile( "Dat_Mon\\dat_brd2.dat", DAT_BOARD2 );
  WEB_INIT_LOG("MainExtendedInitialization: dat_brd2 mounted.");
  LE_DATA_InitDatafile( "Dat_Mon\\dat_3d.dat",   DAT_3D );
  WEB_INIT_LOG("MainExtendedInitialization: dat_3d mounted.");
  ...
  WEB_INIT_LOG("MainExtendedInitialization: initializing language.");
  if (!LANG_InitializeSystem ())
    return FALSE;
  WEB_INIT_LOG("MainExtendedInitialization: language initialized.");

  // Get the font
  WEB_INIT_LOG("MainExtendedInitialization: configuring font.");
  ...
  LE_FONTS_SetSize(10);
  WEB_INIT_LOG("MainExtendedInitialization: font configured.");

  // Initialize the chat system.
  WEB_INIT_LOG("MainExtendedInitialization: initializing chat.");
  if (!CHAT_InitializeSystem ())
    return FALSE;
  WEB_INIT_LOG("MainExtendedInitialization: chat initialized.");

    /* Initialise the messaging system, but do language first since it
  may want to display error messages. */
  WEB_INIT_LOG("MainExtendedInitialization: initializing messaging.");
  if (!MESS_InitializeSystem ())
    return FALSE;
  WEB_INIT_LOG("MainExtendedInitialization: messaging initialized.");
  ...
  WEB_INIT_LOG("MainExtendedInitialization: initializing display.");
  if (!DISPLAY_initialize())
    return(FALSE);
  WEB_INIT_LOG("MainExtendedInitialization: display initialized.");

  /* Get the rules engine going. */
  WEB_INIT_LOG("MainExtendedInitialization: initializing rules.");
  if (!RULE_InitializeSystem ())
    return FALSE;
  WEB_INIT_LOG("MainExtendedInitialization: rules initialized.");

  // Check for command line arguments, like ones which start up
  // networking automatically.
  WEB_INIT_LOG("MainExtendedInitialization: starting networking shim.");
  ShowCursor( TRUE );
  MESS_StartNetworking (LE_MAIN_CommandLine);
  ShowCursor( FALSE );
  WEB_INIT_LOG("MainExtendedInitialization: networking shim done.");
  ...
  UDBOARD_SetBackdrop( DISPLAY_SCREEN_Pselect );
  WEB_INIT_LOG("MainExtendedInitialization: player-select backdrop requested.");
  UDPSEL_SwitchPhase( UDPSEL_PHASE_HISCORE );
  WEB_INIT_LOG("MainExtendedInitialization: player-select phase requested.");

#endif // not USE_OPENING_MOVIES
  WEB_INIT_LOG("MainExtendedInitialization: complete.");
  return TRUE;
}
...
#if MONOPOLY_WEB
    monopoly_web_log("USERIFCE game starting: before UDPSEL_GameHasJustStarted");
#endif
    UDPSEL_GameHasJustStarted();

#if MONOPOLY_WEB
    monopoly_web_log("USERIFCE game starting: setting MainA backdrop");
#endif
    UDBOARD_SetBackdrop( DISPLAY_SCREEN_MainA );
#if MONOPOLY_WEB
    monopoly_web_log("USERIFCE game starting: MainA backdrop set");
#endif
