...
static BOOL RULE_WebShouldLogAction(RULE_ActionType action)
{
  switch (action)
  {
...
static void RULE_WebLogAction(const char *prefix, RULE_ActionArgumentsPointer action)
{
  char buffer[192];
  sprintf(buffer, "%s action=%d from=%d to=%d A=%ld B=%ld C=%ld D=%ld",
    prefix,
    (int) action->action,
    (int) action->fromPlayer,
    (int) action->toPlayer,
    action->numberA,
    action->numberB,
    action->numberC,
...
#if MONOPOLY_WEB
  monopoly_web_log("RULE StartGameInitialisation");
#endif
...
static void ActionRestartPhase (RULE_ActionArgumentsPointer NewActionPntr)
{
  ...
  case GF_CONFIGURATION:
    ...
    {
      char buffer[128];
      sprintf(buffer, "RULE GF_CONFIGURATION pendingSet=0x%lx players=%d",
        (unsigned long) PlayerSet,
        (int) CurrentRulesState.NumberOfPlayers);
      monopoly_web_log(buffer);
    }
#endif
    ...
    if (PlayerSet == 0)
    {
      ...
      {
#if MONOPOLY_WEB
        monopoly_web_log("RULE GF_CONFIGURATION all accepted: starting game");
#endif
...
#if MONOPOLY_WEB
  RULE_WebLogAction("RULE ActionStartGame", NewActionPntr);
#endif
...
static void ActionAcceptConfiguration (RULE_ActionArgumentsPointer NewActionPntr)
{
...
#if MONOPOLY_WEB
  RULE_WebLogAction("RULE ActionAcceptConfiguration", NewActionPntr);
#endif
  ...
  {
    char buffer[192];
    sprintf(buffer, "RULE ActionAcceptConfiguration accepted=%d resend=%d player=%d numberD=%ld",
      CurrentRulesState.Players[NewActionPntr->fromPlayer].shared.starting.acceptedConfiguration,
      NeedToResendNotification,
      (int) NewActionPntr->fromPlayer,
      NewActionPntr->numberD);
    monopoly_web_log(buffer);
  }
#endif
...
#if MONOPOLY_WEB
  if (RULE_WebShouldLogAction(NewActionPntr->action))
  {
    char buffer[224];
    sprintf(buffer, "RULE process action=%d from=%d to=%d A=%ld B=%ld C=%ld D=%ld phase=%d players=%d",
      (int) NewActionPntr->action,
      (int) NewActionPntr->fromPlayer,
      (int) NewActionPntr->toPlayer,
      NewActionPntr->numberA,
      NewActionPntr->numberB,
      NewActionPntr->numberC,
      NewActionPntr->numberD,
      (int) CurrentPhaseM,
      (int) CurrentRulesState.NumberOfPlayers);
    monopoly_web_log(buffer);
